...
function formatAddress(value) {
  ...
  if (typeof value === 'object') {
    const parts = [
      value.logradouro || value.street || value.street_name || '',
      value.numero || value.number || value.street_number || '',
      value.bairro || value.neighborhood || '',
      value.cidade || value.city || '',
      value.estado || value.state || '',
      value.cep || value.postal_code || '',
...
async function loadCompanyData({ tenant, tenantId, apiRequest, getEnvConfig }) {
  ...
  const data = {
    ...
    deliveryInfo: deliveryAreas
      .map((x) => `${x.neighborhood || x.zone_name || 'Região'} (${x.fee})`)
      .join('; '),
    meta: {
