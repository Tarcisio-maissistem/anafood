...
async function saveInboundMessage({
  ...
  saveInboundMessage._doInsert(supabaseUrl, serviceRoleKey, row);
}

// Executa o insert de forma assíncrona sem bloquear
saveInboundMessage._doInsert = async (url, key, row) => {
  try {
 ...
 * Retorna array de { role: 'user', content, at } ordenado por timestamp ASC.
 */
async function loadHistoryMessages({
  ...
  const rows = await supabaseSelectRows({
    ...
    serviceRoleKey,
    table: 'msg_history',
    select: 'history_msg,timestamp,id_msg,name',
    filters,
    order: 'timestamp.asc',
    limit,
  });
...
module.exports = { saveInboundMessage, loadHistoryMessages };
